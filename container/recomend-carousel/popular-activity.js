...
const popularActivity = (link, item) => (<Link href={link} key={item.PackageID} >
    <a alt={item.Name}>
        <div className="card">
            <div className="card-badge">
                <div className="card-badge-left">
                    {
                        item.Rating > 0 ? <>
                            <span className='has-text-primary'><Stars rate={1} max={1} /> {item.Rating}</span>
                            <small className='has-text-white-bis'> | <i className="fas fa-user"></i> {item.BookingCount}</small>
                        </> : <small className='has-text-white-bis'><i className="fas fa-user"></i> {item.BookingCount}</small>
                    }
                </div>
